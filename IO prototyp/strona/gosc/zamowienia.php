...
// 7) Pobierz menu restauracji
$menu = $conn->query("SELECT id_danie, nazwa, cena FROM menu WHERE dostepnosc=1");
?>
<!DOCTYPE html>
<html lang="pl">
<head>
    ...
    <main class="main-content">
      ...
      <?php if(!empty($_SESSION['cart'])): ?>
        <div class="form-container">
          <table>
            ...
            <tbody>
              <?php 
                $total = 0;
                foreach($_SESSION['cart'] as $did => $qty):
                  $row = $conn->query("SELECT nazwa,cena FROM menu WHERE id_danie=$did")
                             ->fetch_assoc();
                  $line = $row['cena'] * $qty;
                  $total += $line;
              ?>
          ...
          <!-- 8) Formularz Zapłać: action z user i order -->
